    ...
    if update.callback_query != None:
        ...
        # keyboard.append(telegram.InlineKeyboardButton("Related Companies", callback_data="RL"))
            ...
            for rl in related_companies:
                if str(rl["companyName"]) != str(company.get("shortName")) and len(keyboard) < 4 and rl["companyName"] not in keyboard:
            ...
            rc_keyboard.pop()
            rc_keyboard.append([telegram.InlineKeyboardButton("⬅️", callback_data="b-{}".format(company.get("symbol")))])
            ...
            company = stockInfo(query[2:])
            caption = "{} | <a href='{}'>Website</a>\nShare Price : {}$ ~ {}\nIndustry : {}\nSector : {}\n\nMore Info ⤵️".format(company.get("shortName"), company.get("website"), company.get("currentPrice"), company.get("exchange"), company.get("industry"), company.get("sector"))
            photo = company.get("logo_url")
            media = telegram.InputMediaPhoto(photo, caption=caption, parse_mode="HTML")

            keyboard = [[telegram.InlineKeyboardButton("📈", callback_data="P-{}".format(query[2:])), telegram.InlineKeyboardButton("🔊", callback_data="V-{}".format(query[2:]))]]
            keyboard.append([telegram.InlineKeyboardButton("Related Companies", callback_data="RC-{}".format(query[2:]))])
            ...
            keyboard = []
            for new in news[0:4]:
                keyboard.append([telegram.InlineKeyboardButton("{}".format(new["publisher"]), url="{}".format(new["link"]))])
            keyboard.append([telegram.InlineKeyboardButton("⬅️", callback_data="B-{}".format(query[2:])), telegram.InlineKeyboardButton("➡️", callback_data="N2-{}".format(query[2:]))])
            keys = telegram.InlineKeyboardMarkup(keyboard)
            bot.edit_message_reply_markup(chat_id=chat_id, message_id=message_id, reply_markup=keys)
            return 'OK'
